public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < nomes.length; i++) t1.inserir(nomes[i]);
        long fimInsercao1 = System.nanoTime();

        long inicioInsercao2 = System.nanoTime();
        for (int i = 0; i < nomes.length; i++) t2.inserir(nomes[i]);
        long fimInsercao2 = System.nanoTime();

        long inicioBusca1 = System.nanoTime();
        for (int i = 0; i < TOTAL_BUSCAS; i++) t1.buscar(nomes[i]);
        long fimBusca1 = System.nanoTime();

        long inicioBusca2 = System.nanoTime();
        for (int i = 0; i < TOTAL_BUSCAS; i++) t2.buscar(nomes[i]);
        long fimBusca2 = System.nanoTime();

        imprimirRelatorio(t1, "Hash 1 (Soma ASCII)", fimInsercao1 - inicioInsercao1, fimBusca1 - inicioBusca1);
        System.out.println("\n" + "=".repeat(60) + "\n");
        imprimirRelatorio(t2, "Hash 2 (Polinomial djb2)", fimInsercao2 - inicioInsercao2, fimBusca2 - inicioBusca2);
    }

    public static void imprimirRelatorio(TabelaHash t, String nome, long tempoIns, long tempoBusca) {
        System.out.println("========================================");
